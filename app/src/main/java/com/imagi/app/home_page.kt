package com.imagi.app

import android.os.Bundle
import android.view.LayoutInflater
import android.view.View
import android.view.ViewGroup
import androidx.fragment.app.Fragment
import androidx.recyclerview.widget.GridLayoutManager
import com.imagi.app.adapter.MarketAdapter
import com.imagi.app.adapter.ProductListAdapter
import com.imagi.app.model.Product
import com.imagi.app.network.Market
import kotlinx.android.synthetic.main.activity_list_market.*
import kotlinx.android.synthetic.main.fragment_home_page.*
import kotlinx.android.synthetic.main.fragment_profile_page.view.*


class HomePage : Fragment() {

    var adapter : ProductListAdapter? = null

    var markets = ArrayList<Market>()

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        rvMarket.layoutManager = GridLayoutManager(this.context, 2)
        var price = 12000
        for (i in 1..10){
            price += i * 10
            var market = Market("Nama Produk $i", price)
            markets.add(market)
        }
        rvMarket.adapter = MarketAdapter(markets)
//        adapter = this.context?.let { ProductListAdapter(it, productList) }

//        gridProduct.adapter = adapter
    }

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? =
        inflater.inflate(R.layout.fragment_home_page, container, false)




    companion object {
        fun newInstance(): HomePage = HomePage()
    }
}